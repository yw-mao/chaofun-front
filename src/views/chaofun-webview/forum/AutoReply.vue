<template>
  <div >
      ...
      <div class="ycontainer">
        <div style="">
          ...
          <div style="margin:10px 0px;display: flex; align-items: center">
            ...
            <input v-model="checkWord"  placeholder="排序值"/>
          </div>
    ...
    <div v-for="(item,index) in lists" :key="index" class="item">
      <div style="max-width: 75%;"> 关键词：{{item.checkWord}} <br/> 自动回复: {{item.responseText}} </div>
...
<script>
  ...
  export default {
    ...
    methods: {
      toAdd() {
        if (this.addRule !== '') {
          api.postByPath('/api/v0/forum/addResponseWord', {forumId: this.forumId, checkWord: this.checkWord, responseText: this.responseText}).then((res) => {
            if (res.success) {
              this.$toast('添加成功')
